...
class DataSyncCore:
    ...
    def __init__(self, data_dir: str = "data", max_retries: int = 3, retry_delay: float = 1):
    ...
    def load_lof_codes(self) -> List[str]:
        """读取LOF代码列表"""
        try:
            with open(PROJECT_ROOT / 'all_LOF.txt', 'r', encoding='utf-8') as f:
    ...
    def load_existing_data(self, code: str) -> pd.DataFrame:
        """加载现有数据"""
        filename = self.data_dir / f"lof_{code}.csv"
        if filename.exists():
            try:
                df = pd.read_csv(filename)
                df['price_dt'] = pd.to_datetime(df['price_dt'])
                for col in ['fund_id', 'code']:
                    if col in df.columns:
                        df[col] = df[col].astype(str)
                numeric_cols = ['price', 'discount_rt', 'net_value', 'volume', 'amount', 'amount_incr', 'amount_increase_rt', 'ref_increase_rt', 'est_val', 'est_val2', 'est_val_increase_rt', 'est_error_rt']
